...
// First-person camera controller - simulates a 1.78m person standing near the UR3e.
//
// Controls:
...
public class FirstPersonCamera : MonoBehaviour
{
    ...
    private int _skipLookFrames = 0;   // guard: ignore mouse delta for N frames after re-locking
    ...
    void LockCursor()
    {
        ...
        cursorLocked = true;
        _skipLookFrames = 2;   // discard the large delta from cursor-to-centre snap
    }
    ...
    void HandleCursorLock()
    {
        ...
        if (keyboard.escapeKey.wasPressedThisFrame)
            UnlockCursor();
        else if (!cursorLocked && mouse.leftButton.wasPressedThisFrame && !IsPointerOverUI())
            LockCursor();
#else
        if (Input.GetKeyDown(KeyCode.Escape))
            UnlockCursor();
        else if (!cursorLocked && Input.GetMouseButtonDown(0) && !IsPointerOverUI())
            LockCursor();
#endif
    }

    bool IsPointerOverUI() => HUDController.IsPointerOver;

    // Look (mouse)

    void HandleLook()
    {
        // Skip delta for a couple of frames after re-locking to avoid the view
        // snapping when the cursor jumps back to screen centre.
        if (_skipLookFrames > 0) { _skipLookFrames--; return; }
